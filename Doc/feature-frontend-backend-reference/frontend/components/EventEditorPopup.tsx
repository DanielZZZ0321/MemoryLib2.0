        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          ...
          <section className="space-y-2">
            <h3 className="flex items-center gap-2 text-xs font-medium text-slate-400 uppercase tracking-wider">
              <Clock className="w-3.5 h-3.5" /> 鍏冩暟鎹?            </h3>
            <div className="grid grid-cols-2 gap-3">
              {mode === 'edit' ? (
                <>
                  <div>
                    <label className="block text-xs text-slate-500 mb-1">鏍囬</label>
                    <input
                      type="text"
                      value={event.title}
                  ...
                  <div>
                    <label className="block text-xs text-slate-500 mb-1">寮€濮嬫椂闂?/label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="block text-xs text-slate-500 mb-1">缁撴潫鏃堕棿</label>
                    <input
                      type="text"
                      value={event.end_hms}
                  ...
                  <div>
                    <label className="block text-xs text-slate-500 mb-1">鏍囩 (閫楀彿鍒嗛殧)</label>
                    <input
                      type="text"
                ...
                <div className="col-span-2 flex flex-wrap gap-2 text-sm">
                  <span className="text-slate-400">浜嬩欢 #{eventIndex + 1}</span>
                  <span className="text-slate-500">路</span>
                  <span className="text-slate-400">{event.start_hms} 鈥?{event.end_hms}</span>
          ...
          <section className="space-y-2">
            <h3 className="flex items-center gap-2 text-xs font-medium text-slate-400 uppercase tracking-wider">
              <FileText className="w-3.5 h-3.5" /> 鎽樿
            </h3>
            {mode === 'edit' ? (
              <textarea
                value={event.summary}
              ...
              <p className="text-slate-300 text-sm leading-relaxed">{event.summary || '鈥?}</p>
            )}
          </section>
          ...
          <section className="space-y-2">
            <h3 className="flex items-center gap-2 text-xs font-medium text-slate-400 uppercase tracking-wider">
              <FileText className="w-3.5 h-3.5" /> 绗旇
            </h3>
            {mode === 'edit' ? (
              <textarea
                value={event.notes ?? ''}
              ...
              <p className="text-slate-300 text-sm leading-relaxed whitespace-pre-wrap">{event.notes || '鈥?}</p>
            )}
          </section>
          ...
          <section className="space-y-2">
            <h3 className="flex items-center gap-2 text-xs font-medium text-slate-400 uppercase tracking-wider">
              <Video className="w-3.5 h-3.5" /> 瑙嗛
            </h3>
            {videos.length > 0 ? (
              ...
              <p className="text-slate-500 text-sm">{mode === 'edit' ? '鏆傛棤瑙嗛锛屽彲鍦?media 鏁扮粍涓坊鍔?type: "video" 鐨勯」' : '鏆傛棤瑙嗛'}</p>
            )}
          </section>
          ...
          <section className="space-y-2">
            <h3 className="flex items-center gap-2 text-xs font-medium text-slate-400 uppercase tracking-wider">
              <Music className="w-3.5 h-3.5" /> 闊抽
            </h3>
            {audios.length > 0 ? (
              <div className="space-y-2">
                {audios.map((m, i) => (
              ...
              <p className="text-slate-500 text-sm">{mode === 'edit' ? '鏆傛棤闊抽銆傚湪 media 涓坊鍔?type: "audio" 鐨勯」浠ユ樉绀? : '鏆傛棤闊抽'}</p>
            )}
          </section>
          ...
          <section className="space-y-2">
            <h3 className="flex items-center gap-2 text-xs font-medium text-slate-400 uppercase tracking-wider">
              <Image className="w-3.5 h-3.5" /> 鐓х墖
            </h3>
            {photos.length > 0 ? (
              ...
              <p className="text-slate-500 text-sm">鏆傛棤鐓х墖</p>
            )}
          </section>
        </div>
