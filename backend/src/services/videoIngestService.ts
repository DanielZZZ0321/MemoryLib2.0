...
function runVideoTimelineAnalysis(videoPath: string): { ok: true; events: TimelineEvent[]; raw: unknown } | { ok: false; error: string } {
  ...
  if (!fs.existsSync(scriptPath)) return { ok: false, error: '视频分析脚本不存在' };
  ...
  try {
    ...
    if (!json.success) return { ok: false, error: json.error || '分析失败' };
    const events = Array.isArray(json.events) ? json.events : [];
    return { ok: true, events, raw: json };
  } catch {
    return { ok: false, error: stdout || stderr || '解析分析结果失败' };
  }
}
