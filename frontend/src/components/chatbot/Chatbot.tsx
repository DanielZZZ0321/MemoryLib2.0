...
export function Chatbot() {
  ...
  const handleDrop = (e: React.DragEvent) => {
    ...
    if (eventData) {
      try {
        ...
        if (!attachedEvents.find((e) => e.id === event.id)) {
  ...
  const handleSend = async () => {
    ...
    const eventIds = attachedEvents.map((e) => e.id);

    await sendMessage(inputValue, eventIds, attachedEvents);
            ...
            {messages.length === 0 ? (
              <div className="h-full flex flex-col items-center justify-center text-center text-zinc-500 dark:text-zinc-400">
                ...
                <p className="text-xs max-w-[200px]">
                  Drag events here or type a message. I can help you summarize, search, and organize your memories.
                </p>
              </div>
            ) : (
                  ...
                  >
                    ...
                    <div
                      className={`text-xs mt-2 flex items-center gap-1 ${
                        message.role === 'user' ? 'text-white/60' : 'text-zinc-400'
                      }`}
                    >
            ...
            <div className="px-4 py-2 border-t border-zinc-200 dark:border-zinc-800 bg-zinc-100 dark:bg-zinc-900">
              ...
              <div className="flex flex-wrap gap-2">
                {attachedEvents.map((event) => (
                  <div
                    key={event.id}
                    className="flex items-center gap-1 bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-lg px-2 py-1 text-xs"
                  >
            ...
            <div className="flex gap-2">
              <div className="flex-1 relative">
                <textarea
                  ...
                  rows={1}
                  className="w-full bg-zinc-100 dark:bg-zinc-800 border border-transparent focus:border-blue-500/50 rounded-xl px-4 py-3 text-sm text-zinc-900 dark:text-zinc-100 placeholder:text-zinc-400 resize-none focus:outline-none focus:ring-2 focus:ring-blue-500/30"
                  style={{
                    minHeight: '44px',
                    maxHeight: '120px',
                  }}
              ...
              <button
                ...
                className="w-11 h-11 bg-blue-500 hover:bg-blue-600 disabled:bg-zinc-300 dark:disabled:bg-zinc-700 text-white rounded-xl flex items-center justify-center transition-colors disabled:cursor-not-allowed"
              >
    ...
    <>
      {lines.map((line, i) => {
        ...
        if (numberedMatch) {
          return (
            <p key={i} className="pl-2">
              {numberedMatch[1]}. {renderBoldText(line.slice(numberedMatch[0].length))}
            </p>
          );
        }
